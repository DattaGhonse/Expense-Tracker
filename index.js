...
let tlistEl = document.querySelector(".tlist");
...
tForm.onsubmit = (e) =>{
    ...
    swal("success","tranasaction added", 'success');
    btnClose.click();
    tForm.reset('');
    showTransaction();
    calculation();
...
// delete transaction
const deleteFunc = () =>{
    let alDelBtn = tlistEl.querySelectorAll(".del-btn");
    alDelBtn.forEach((btn,index) =>{
        btn.onclick = () =>{
            swal({
                title: "Are you sure?",
        ...
        btn.onclick = () =>{
            ...
            allBtn[1].classList.remove("d-none");
             allBtn[1].onclick = () =>{
                ...
                localStorage.setItem('transaction',JSON.stringify(transaction));
                swal("success","tranasaction Updated", 'success');
                btnClose.click();
                tForm.reset('');
                showTransaction();
                calculation();
...
incomeEl.innerHTML =  `₹${totalCr}`;
expenseEl.innerHTML = `₹${totalDr}`;
Number(totalCr-totalDr) < 0 ? balanceEl.style.color = 'red' : balanceEl.style.color = 'green';
balanceEl.innerHTML = `₹${Number(totalCr-totalDr)}`;
